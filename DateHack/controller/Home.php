...
class Home 
{
	...
	function userLogin($userEmail, $userPassword){
		if(!empty($userEmail) && !empty($userPassword)){
			if($this->db->veryfyUser($userEmail)==="userErrror"){
				return "No User found.Please register";
			}
			else{
				$userDetails =  $this->db->veryfyUser($userEmail);
				if(password_verify($userPassword, $userDetails['password'])){
	...
	function zodiacFinder($birthdate) {
   		$zodiac = '';     
